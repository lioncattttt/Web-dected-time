...
import av  # <-- FIX 1: Must import av directly for frame conversion
...
CANDIDATE_WEIGHTS = ["best.pt", "best-5.pt"]
    ...
    class VideoTransformer(VideoProcessorBase):
        ...
        def recv(self, frame):
            ...
            if results[0].boxes is not None and len(results[0].boxes) > 0:
                ...
                cls_id = int(box_obj.cls[0].cpu().item())
                cls_name = model_class_names.get(cls_id, "unknown").lower()
                ...
                self.pose_history.append({'y': center_y, 't': current_time})
                
                while self.pose_history and (current_time - self.pose_history[0]['t'] > self.time_window_ms):
                    self.pose_history.popleft()
                
                if len(self.pose_history) > 3:
                    delta_y = self.pose_history[-1]['y'] - self.pose_history[0]['y']
                    delta_t = (self.pose_history[-1]['t'] - self.pose_history[0]['t']) / 1000.0
                    
                    velocity_y = delta_y / delta_t if delta_t > 0 else 0
            ...
            cv2.rectangle(annotated_frame, (20, 20), (450, 65), (29, 20, 17), -1)
